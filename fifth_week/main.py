...
def get_min_index(arr: list, start: int = 0):
    min_value = None
    min_index = None
    for i in range(start, len(arr)):
        if i == start or arr[i] < min_value:
            min_value = arr[i]
            min_index = i
    return min_index


def ex_selection_sort(arr: list):
    new_arr = arr.copy()
    for i in range(len(new_arr)):
        min_index = get_min_index(new_arr, i)
        new_arr[i], new_arr[min_index] = new_arr[min_index], new_arr[i]
    return new_arr


def ex_insertion_sort(arr: list):
    new_arr = arr.copy()
    for i in range(1, len(new_arr)):
        j = i - 1
        current = new_arr[i]
        while j >= 0 and current < new_arr[j]:
...
def get_min(arr: list, predicate = lambda val: val, start: int = 0):
    min_value = None
    min_index = None
    for i in range(start, len(arr)):
        if i == start or predicate(arr[i]) < predicate(min_value):
            min_value = arr[i]
            min_index = i
    return min_index
...
def selection_sort(arr: list):
    new_arr = arr.copy()
    for i in range(len(new_arr)):
        min_index = get_min(new_arr, lambda val: val['nome'], i)
        new_arr[i], new_arr[min_index] = new_arr[min_index], new_arr[i]
    return new_arr
...
def insertion_sort(arr: list):
    ...
    for i in range(1, len(new_arr)):
        j = i - 1
        current = new_arr[i]
        while j >= 0 and new_arr[j]['nome'] > current['nome']:
            new_arr[j+1] = new_arr[j]
            j -= 1
        new_arr[j+1] = current
    return new_arr
...
# print(*selection_sort(servidores_dns), sep='\n')
print(*insertion_sort(servidores_dns), sep='\n')
